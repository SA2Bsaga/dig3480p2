...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
  ...
  var playerg = this.level.player;
  // Change coordinates from the source to our scaled.
  var center = playerg.pos.plus(playerg.size.times(0.5))
                 .times(scale);
...
Spy.prototype.act = function(step) {
	
	var newPos = this.pos.plus(this.speed.times(step));
	this.pos = newPos;
	/*var targetX = 25 - this.pos.x;
        var targetY = 0 - this.pos.y;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if ((type == "lava" && this.status == null) || (type == "spy" && this.status == null)) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
